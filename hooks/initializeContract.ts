...
export function useInitializeContract() {
  ...
  const deployProxyContract = async (data: unknown): Promise<Result[]> => {
    if (!provider) {
      const { provider: newProvider, signer: newSigner } = await connect();
      update_provider(newProvider);
      update_signer(newSigner);
    }
    ...
    const proxy_contract = new Contract(proxy_address, proxy_abi, signer);
  ...
  return { deployProxyContract, connectProxyContract, signContract };
}
